...
struct OptimiserWeeklyDeviationTests {
    ...
    @Test("One historical week cannot flip a late-week deficit positive")
    func singleHistoricalWeekDoesNotOverrideLateDeficit() {
        let now = Date()
        let currentWeeklyRemaining = 500.0
        let currentElapsedMinutes = UsageOptimiser.weekMinutes - currentWeeklyRemaining
        let currentWeekStart = now.addingTimeInterval(-currentElapsedMinutes * 60)
        let oldWeekStart = currentWeekStart.addingTimeInterval(-7 * 86400)
        let oldLate = oldWeekStart.addingTimeInterval(currentElapsedMinutes * 60)

        let data = makeStoreData(
            polls: [
                (oldWeekStart, 0, 300, 0, UsageOptimiser.weekMinutes),
                (oldLate.addingTimeInterval(-1200), 8, 280, 60, 520),
                (oldLate.addingTimeInterval(-600), 12, 270, 60, 510),
                (oldLate, 16, 260, 60, 500),
                (oldLate.addingTimeInterval(600), 20, 250, 60, 490),
                (oldLate.addingTimeInterval(1200), 24, 240, 60, 480),
                (currentWeekStart, 0, 300, 0, UsageOptimiser.weekMinutes),
            ]
        )
        let opt = makeTestOptimiser(data: data)

        let result = opt.recordPoll(
            sessionUsage: 20, sessionRemaining: 240,
            weeklyUsage: 87, weeklyRemaining: currentWeeklyRemaining,
            timestamp: now
        )
        #expect(result.weeklyDeviation < 0)
    }

    @Test("Empirical history can soften but not reverse a late-week deficit")
    func empiricalHistoryStaysSubordinateToLateDeficit() {
        let now = Date()
        let currentWeeklyRemaining = 500.0
        let currentElapsedMinutes = UsageOptimiser.weekMinutes - currentWeeklyRemaining
        let currentWeekStart = now.addingTimeInterval(-currentElapsedMinutes * 60)

        func lateWeek(_ weekStart: Date, usage: Double) -> [(Date, Double, Double, Double, Double)] {
            let late = weekStart.addingTimeInterval(currentElapsedMinutes * 60)
            return [
                (weekStart, 0, 300, 0, UsageOptimiser.weekMinutes),
                (late.addingTimeInterval(-600), 10, 270, usage, 510),
                (late, 14, 260, usage, 500),
                (late.addingTimeInterval(600), 18, 250, usage, 490),
            ]
        }

        let data = makeStoreData(
            polls:
                lateWeek(currentWeekStart.addingTimeInterval(-21 * 86400), usage: 60)
                + lateWeek(currentWeekStart.addingTimeInterval(-14 * 86400), usage: 63)
                + lateWeek(currentWeekStart.addingTimeInterval(-7 * 86400), usage: 32)
                + [(currentWeekStart, 0, 300, 0, UsageOptimiser.weekMinutes)]
        )
        let opt = makeTestOptimiser(data: data)
